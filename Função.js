...
window.dalay = function (ms) { // Dalay para async functions
    return new Promise(resolve => setTimeout(resolve, ms));
}

window.CriarBotão = function (texto, titulo, id, elementopai, classe) { // função para criar botão
...
window.print = function (elemento, nome="captura.png") { // Tira "foto" da tabela
...
window.alert = async function (texto){ // substituir alert()
    ...
    balão.querySelector("p").innerText = texto
    await dalay(5000)

    if (balão.style.opacity == 0) {
        fechar.removeEventListener('click', fecharBalao);
        return;
    }
...
window.ApertouBotão = function() { // função de quando cada botão do editor é apertado
    ...
    for (let index = 0; index < arraychavesusadas.length; index++) { // loop para definir botão de mover cabeças <
        ...
        botãocabeça.addEventListener('click', function(){
            ...
            window.RecarregarTabela("Coluna movida!")
        })
    }
    ...
    for (let index = 2; index < json.length; index++) { // loop para definir botão de mover linha ^
        ...
        botãolinha.addEventListener('click', function(){
            ...
            window.RecarregarTabela("Linha movida!")
        })
    }
